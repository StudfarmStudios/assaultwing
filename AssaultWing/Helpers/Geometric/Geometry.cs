...
namespace AW2.Helpers.Geometric
{
    ...
    public class Geometry
    {
        ...
        /// <summary>
        /// Kinds of intersection volumes that a line and a cone have.
        /// </summary>
        public enum LineConeIntersectionType
        {
            /// <summary>
            /// The line and the cone don't intersect.
            /// </summary>
            None,

            /// <summary>
            /// The line is tangent to the cone at a single point.
            /// </summary>
            Point,

            /// <summary>
            /// The line strictly penetrates the cone.
            /// </summary>
            Segment,

            /// <summary>
            /// The line passes through the tip of the cone and into the cone.
            /// </summary>
            Ray,
        }
        ...
        /// <summary>
        /// Returns the type of intersection of a line and a cone along with information
        /// about the geometry of the intersection.
        /// </summary>
        /// <param name="coneDirection">Points from the cone tip into the cone.</param>
        /// <param name="squareCosConeHalfCenterAngle">Squared cosine of half of the center angle of the cone.</param>
        /// <param name="result1">Information about the intersection volume, interpreted depending
        /// on the method's return value.</param>
        /// <param name="result2">Information about the intersection volume, interpreted depending
        /// on the method's return value.</param>
        /// <remarks>
        /// <paramref name="result1"/> and <paramref name="result2"/> are interpreted as follows.
        /// <see cref="LineConeIntersectionType.None"/>: empty array.
        /// <see cref="LineConeIntersectionType.Point"/>: result1 is the intersection point.
        /// <see cref="LineConeIntersectionType.Segment"/>: (result1, result2) is the intersection segment.
        /// <see cref="LineConeIntersectionType.Ray"/>: result1 + t * result2 is the intersection ray.
        /// </remarks>
        public static LineConeIntersectionType Intersect(Vector3 coneTip, Vector3 coneDirection, float squareCosConeHalfCenterAngle,
            Vector3 linePoint, Vector3 lineDirection, out Vector3 result1, out Vector3 result2)
        {
            // This algorithm is adapted from Wild Magic source code (http://www.geometrictools.com/),
            // licensed under the Boost License 1.0. See "Boost Software License 1_0.txt" in the root
            // of the Assault Wing source code tree.
            ...
            float AdD = Vector3.Dot(coneDirection, lineDirection);
            var E = linePoint - coneTip;
            float AdE = Vector3.Dot(coneDirection, E);
            float DdE = Vector3.Dot(lineDirection, E);
            float EdE = Vector3.Dot(E, E);
            float c2 = AdD * AdD - squareCosConeHalfCenterAngle;
            float c1 = AdD * AdE - squareCosConeHalfCenterAngle * DdE;
            float c0 = AdE * AdE - squareCosConeHalfCenterAngle * EdE;
            const float ZERO_TOLERANCE = 0.00001f;
            float dot;
            LineConeIntersectionType intersectionType;
            result1 = Vector3.Zero;
            result2 = Vector3.Zero;

            // Solve the quadratic.  Keep only those X for which Dot(A,X-V) >= 0.
            if (Math.Abs(c2) >= ZERO_TOLERANCE)
            {
                // c2 != 0
                float discr = c1 * c1 - c0 * c2;
                if (discr < 0)
                {
                    // Q(t) = 0 has no float-valued roots.  The line does not
                    // intersect the double-sided cone.
                    intersectionType = LineConeIntersectionType.None;
                }
                else if (discr > ZERO_TOLERANCE)
                {
                    // Q(t) = 0 has two distinct float-valued roots.  However, one or
                    ...
                    result1 = linePoint + t * lineDirection;
                    E = result1 - coneTip;
                    dot = Vector3.Dot(E, coneDirection);
                    int resultCount = 0;
                    if (dot > 0) resultCount++;

                    t = (-c1 + root) * invC2;
                    var potentialResult = linePoint + t * lineDirection;
                    E = potentialResult - coneTip;
                    if (resultCount == 0) result1 = potentialResult;
                    else result2 = potentialResult;
                    dot = Vector3.Dot(E, coneDirection);
                    if (dot > 0) resultCount++;
                    ...
                    switch (resultCount)
                    {
                        case 2:
                            ...
                            // vertex twice.
                            intersectionType = LineConeIntersectionType.Segment;
                            break;
                        case 1:
                            // The line intersects the single-sided cone in front of the
                            // vertex once.  The other intersection is with the
                            // single-sided cone behind the vertex.
                            intersectionType = LineConeIntersectionType.Ray;
                            result2 = lineDirection;
                            break;
                        default:
                ...
                {
                    // One repeated float root (line is tangent to the cone).
                    result1 = linePoint - (c1 / c2) * lineDirection;
                    E = result1 - coneTip;
                    if (Vector3.Dot(E, coneDirection) > 0)
                        intersectionType = LineConeIntersectionType.Point;
                    else
                        intersectionType = LineConeIntersectionType.None;
                }
            }
            else if (Math.Abs(c1) >= ZERO_TOLERANCE)
            {
                // c2 = 0, c1 != 0 (D is a direction vector on the cone boundary)
                result1 = linePoint - (0.5f * c0 / c1) * lineDirection;
                E = result1 - coneTip;
                dot = Vector3.Dot(E, coneDirection);
                if (dot > 0)
                {
            ...
            else if (Math.Abs(c0) >= ZERO_TOLERANCE)
            {
                // c2 = c1 = 0, c0 != 0
                intersectionType = LineConeIntersectionType.None;
            }
        ...
        public class GeometryTest
        {
            ...
            [Test]
            public void TestIntersectLineCone()
            {
                var coneTip = new Vector3(10, 10, 10);
                var coneDirection = Vector3.UnitZ;
                float cosConeHalfCenterAngle = (float)(1 / Math.Sqrt(2)); // 90 degree center angle
                ...
                // Line passes through cone tip.
                // Note: The algorithm seems to give no intersection if the line
                // passes through the cone tip, even if the line otherwise goes inside the cone.
                // If this becomes a problem, fix the algorithm.
                Assert.AreEqual(LineConeIntersectionType.None,
                    Intersect(coneTip, coneDirection, cosConeHalfCenterAngle,
                ...
                Assert.AreEqual(LineConeIntersectionType.Ray,
                    Intersect(coneTip, coneDirection, cosConeHalfCenterAngle,
                    Vector3.Zero, coneDirection,
                    out result1, out result2));
                AssertAreInDelta(new Vector3(0, 0, 10 + (float)Math.Sqrt(10 * 10 + 10 * 10)), result1, SMALL_VALUE);
                AssertAreInDelta(coneDirection, result2, SMALL_VALUE);
            }
