...
float4 bloomEffectH(PS_INPUT Input) : COLOR0 { 
...
float4 bloomEffectV(PS_INPUT Input) : COLOR0 { 
   // float4 sum = tex2D(samplerState, Input.TexCoord);
    float4 sum = (0,0,0,0);
    float4 tex;
...
technique Bloom
{
    pass P0
    {
        PixelShader = compile ps_2_0 bloomEffectH();
    }
    pass P1
    {
        PixelShader = compile ps_2_0 bloomEffectV();
    }
}
